...
╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝╚═╝     ╚═╝╚══════╝╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝
    ...
    print("─" * 125)
    print(f" {C_PINK}[Module]{RESET}                -> Animexin A-Z List Scraper Core")
    print(f" {C_PINK}[Target Endpoint]{RESET}       -> https://animexin.dev/az-lists/")
...
async def scrape_az_list(show_param: str, page: int):
    base_url = "https://animexin.dev/az-lists/"
    
    encoded_show = quote(show_param)
    query_str = f"?show={encoded_show}" if show_param else ""
    
    if page > 1:
        target_url = f"https://animexin.dev/az-lists/page/{page}/{query_str}"
    else:
    ...
    response = {
        ...
        "message": f"Mantap! Berhasil menarik data A-Z List Animexin filter '{show_param}' halaman {page}." if status_code == 200 else f"Gagal mengambil halaman (Status Code: {status_code})",
        "ok": status_code == 200,
    ...
    if status_code != 200 or not html_content:
        response["message"] = f"Duh, zonk bro! Halaman A-Z Animexin untuk filter '{show_param}' tidak ditemukan (Status Code: {status_code})."
        response["ok"] = False
        return response
    ...
    try:
        ...
        response["message"] = f"Waduh ada error saat parsing HTML Animexin: {str(e)}"
        response["ok"] = False
...
async def main():
    ...
    print(f"\n{C_PINK}[?]{RESET} {C_BLUE}Pilih Filter A-Z List Animexin:{RESET}")
    ...
    print()
    await loading_animation(f"Menghubungkan ke Animexin A-Z List [Filter: {show_param}]", 0.3)
    await loading_animation(f"Mengambil data halaman ke-{page}", 0.4)
    
    print(f"\n{C_PINK}[*]{RESET} {C_BLUE}Processing Animexin A-Z payload extraction...{RESET}")
