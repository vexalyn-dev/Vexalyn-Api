...
╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝ ╚═════╝╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝
 ...
 ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
 [Module]                -> All Seasons Scraper Core (season.py)
...
async def scrape_all_seasons():
    ...
    if not html_content or status_code != 200:
        ...
        if err_code == 403 or (html_content and ("Just a moment" in html_content or "cf-challenge" in html_content.lower())):
            ...
            err_msg = f"Gagal narik data season, server merespon dengan status code {err_code}."
    ...
    season_elements = soup.select('input[name*="season"], .filter-season input, input[name="season[]"], .filter-row input, ul.season li a, a[href*="season="]')
    
    seasons_list = [{"name": "All", "slug": "all"}]
    seen_slugs = {"all"}
    ...
    for el in season_elements:
        if el.name == 'input':
            ...
            if 'season=' in raw_val:
                raw_val = raw_val.split('season=')[-1].split('&')[0]
            else:
    ...
    return {
        ...
        "statusCode": 200,
        "status": "success",
        "message": f"Mantap! Berhasil menarik {total_seasons} daftar season dari Anichin.",
        "elapsed_time": f"{elapsed} seconds",
...
async def main():
    ...
    await loading_animation("Lagi nyari data filter season", 0.5)
    ...
    if result["statusCode"] == 200:
        ...
        print(f"{C_PINK}[SUCCESS]{RESET} {C_BLUE}Total Seasons:{RESET} {C_CYAN}{result['total_seasons']} seasons extracted{RESET}")
